...
union_qa_error_postivate = pd.concat([default_df, union_qa_data.drop(['seq', 'new_temp_index'], axis=1)], axis=0)

union_qa_error_postivate = union_qa_error_postivate.sample(frac=1)  # 乱序处理
union_qa_error_postivate.to_json('data/origin/intercontest/culture_qa_error_postivate_train.json', force_ascii=False,orient='records', lines=True)
